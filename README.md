...
| **Stop**（入力待ち） | codex CLI がある場合は最後の出力を15文字以内に要約して吹き出し表示、なければ「入力を待っているのだ！」（右クリックメニューで閉じる） |
...
全メッセージタイプ（Permission・Notification・Stop）で、ずんだもんの足元に黒背景＋テーマカラーのステータステキストが表示されます。Claude Code が今何をしているかがリアルタイムにわかります。
