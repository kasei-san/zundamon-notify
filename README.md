...
│   └── marker-files.js        # マーカーファイルによるAgent状態管理
├── assets/
...
| **RISK** | `⚠️RISK: リスクの理由` + 従来の Y/N ボタン | AWS 破壊的操作、terraform apply/destroy、git push --force、rm -rf、sudo、DB DROP/TRUNCATE |
| **UNKNOWN** | `❓判定不能なのだ` + 従来の Y/N ボタン | codex の応答遅延、形式エラー、ネットワークエラー等 |

### ログ

全判定結果（SAFE/RISK/UNKNOWN）が JSON Lines 形式でログに記録されます：
```jsonl
{"timestamp":"2026-02-26T10:30:00+00:00","judgment":"SAFE","tool_name":"Bash","description":"ls -la","summary":"ファイル一覧を確認するのだ","cwd":"/Users/you/work/project","session_id":"session-abc"}
{"timestamp":"2026-02-26T10:31:00+00:00","judgment":"RISK","tool_name":"Bash","description":"rm -rf /tmp/*","summary":"一時ファイルを全削除するのだ","cwd":"/Users/you/work/project","session_id":"session-abc"}
```

判定別にログを抽出：
```bash
grep '"judgment":"RISK"' ~/.config/zundamon-notify/auto-approve.log
...
Hook スクリプトは UDS メッセージ送信に加えて、`/tmp/zundamon-markers/<session_id>/` 配下にマーカーファイルを作成します。これにより、hooks 非依存でエージェントの状態遷移を外部から追跡できます。

### ディレクトリ構造

```
/tmp/zundamon-markers/
└── <session_id>/
    ├── .prompt_submitted   # UserPromptSubmit時に作成
    └── .agent_stopped      # Stop時に作成
```

### 状態遷移

| `.prompt_submitted` | `.agent_stopped` | 状態 | 意味 |
|---------------------|-----------------|------|------|
| なし | なし | `idle` | 待機中 |
| あり | なし | `working` | エージェント作業中 |
| あり | あり | `waiting` | 入力待ち |

### ライフサイクル

1. **UserPromptSubmit**: `.prompt_submitted` を作成、`.agent_stopped` を削除 → `working`
2. **Stop**: `.agent_stopped` を作成 → `waiting`
3. **SessionEnd**: ディレクトリごと削除 → `idle`

### 外部からの状態確認

```bash
# 特定セッションの状態を確認
ls /tmp/zundamon-markers/<session_id>/

# 全セッションの状態を一覧
for d in /tmp/zundamon-markers/*/; do
  session=$(basename "$d")
  if [ -f "$d/.prompt_submitted" ] && [ ! -f "$d/.agent_stopped" ]; then
    echo "$session: working"
  elif [ -f "$d/.prompt_submitted" ] && [ -f "$d/.agent_stopped" ]; then
    echo "$session: waiting"
  else
    echo "$session: idle"
  fi
done
...
この仕組みは [takkota/my-agents](https://github.com/takkota/my-agents) のマーカーファイル方式にインスパイアされています。Claude Code / Codex / Gemini 等、特定の hook システムに依存しない共通プロトコルとして利用可能です。
