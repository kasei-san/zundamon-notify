...
│   └── zundamon-codex-notify.sh # Codex CLI notify hook（入力待ち通知）
├── .claude/
│   └── skills/
...
[Codex CLI](https://github.com/openai/codex) の `notify` イベントにも対応しています。Codex が応答を完了してユーザー入力待ちになった時に、ずんだもんが通知します。
...
```toml
notify = ["/path/to/zundamon-notify/hooks/zundamon-codex-notify.sh"]
```

> パスは実際のインストール先に合わせて変更してください（例: `~/work/zundamon-notify/hooks/zundamon-codex-notify.sh`）。

### 動作

Codex が `agent-turn-complete` イベントを発火すると、スクリプトが JSON 引数（`$1`）からイベント情報を取得し、UDS 経由でずんだもん通知アプリに `stop` メッセージを送信します。

- **session_id**: `codex-<cwdのbasename>` 形式（例: `codex-my-project`）で、Claude Code セッションとは別のずんだもんウィンドウとして表示されます
- **メッセージ**: 「Codexが入力を待っているのだ！」
