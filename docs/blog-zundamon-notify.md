...
> **注意: この機能はデフォルトでオフです。** LLM（codex CLI）にリスク判定を委ねるため、**誤判定のリスクがあります**。安全なコマンドが RISK と判定される（偽陽性）場合は従来通り手動確認になるだけですが、**危険なコマンドが SAFE と判定される（偽陰性）場合はユーザー確認なしに実行されます**。判定ログ（`~/.config/zundamon-notify/auto-approve.log`）を定期的に確認し、意図しない自動許可がないかチェックすることを推奨します。本番環境や機密データを扱う作業では無効のままにしておくのが安全です。この機能のリスクを理解した上で、自己責任でご利用ください。
>
> **推奨する使い方:** まずは Claude Code 本体の `settings.json` の `allow` ルールで、確実に安全なコマンド（`git status`、`ls`、Read ツール等）を確定的に許可設定してください。LLM による自動判定はあくまで補助であり、allow ルールでカバーしきれないグレーゾーンの判定を任せる、という位置づけで使うのが安全です。
