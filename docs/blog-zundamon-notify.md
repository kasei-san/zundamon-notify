...
> **注意**: この機能は LLM（codex CLI）にリスク判定を委ねるため、**誤判定のリスクがあります**。安全なコマンドが RISK と判定される（偽陽性）場合は従来通り手動確認になるだけですが、**危険なコマンドが SAFE と判定される（偽陰性）場合はユーザー確認なしに実行されます**。判定ログ（`~/.config/zundamon-notify/auto-approve.log`）を定期的に確認し、意図しない自動許可がないかチェックすることを推奨します。本番環境や機密データを扱う作業では無効のままにしておくのが安全です。この機能のリスクを理解した上で、自己責任でご利用ください。
