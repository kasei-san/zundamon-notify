...
# PR監視テスト
echo '{"type":"pr_monitor","id":"pr-test","session_id":"session-aaa","url":"https://github.com/owner/repo/pull/123"}' | socat -t 2 - UNIX-CONNECT:/tmp/zundamon-claude.sock
...
`/tmp/zundamon-claude.sock` で JSON Lines プロトコルを処理。`sessions: Map<session_id, {pid, cwd, pendingConnections}>` でセッション単位管理。コールバック方式（`onMessage`, `onSessionStart`, `onSessionEnd`, `onPermissionRequest`, `onSessionPermissionsDismiss`, `onAllPermissionsDismiss`, `onPrMerged`）で main.js が適切なウィンドウにルーティング。`session_id` 未設定メッセージは `"default"` にフォールバック。DISMISS ハンドラは `getOrCreateSession` を呼ぶため、UserPromptSubmit hook での最初の dismiss メッセージでセッションが自動作成される。`PrMonitor` を内包し、`pr_monitor` メッセージ受信時にPR監視を登録、セッション終了時に該当セッションのPR監視を解除する。

### PRモニター (`src/pr-monitor.js`)
GitHub PRの状態をポーリングし、マージされたらコールバックで通知する。`addPr(url, sessionId)` で監視登録、`removePr(url)` / `removeBySession(sessionId)` で解除。60秒間隔（設定可能）で `gh pr view` コマンドを実行し、`MERGED` 状態を検知したら自動的に監視リストから削除してコールバック呼び出し。URL解析は `https://github.com/<owner>/<repo>/pull/<number>` 形式に対応。

### プロトコル (`src/protocol.js`)
メッセージ型（`PERMISSION_REQUEST`, `NOTIFICATION`, `STOP`, `DISMISS`, `STATUS_UPDATE`, `SESSION_END`, `PR_MONITOR`）の定義とパース/シリアライズ。`session_id` 未設定時は `"default"` にフォールバック。
