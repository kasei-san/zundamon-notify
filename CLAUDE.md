...
Codex CLI 連携: **Codex CLI notify → `zundamon-codex-notify.sh`（$1でJSON受信） → UDS → Electron main → IPC → renderer**。Codex の `agent-turn-complete` イベントを `stop` メッセージに変換。`session_id` は `codex-<cwdのbasename>` 形式で Claude Code セッションと区別。
